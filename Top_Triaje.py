...
class Triaje(object):
	...
	def Top_Agregar(self,cupo,servicio,medico,usuario,fecha_Atencion,turno):
		self.turno=turno
		self.usuario=usuario
		self.etiqueta_Cupo=cupo		
		self.ventana_Triaje=Toplevel()
		self.ventana_Triaje.geometry('700x400+0+100')
		...
		self.btn_Guardar=ttk.Button(self.ventana_Triaje,text='GUARDAR')
		self.btn_Guardar['command']=self.registrar_cita
		self.btn_Guardar.grid(row=10,column=1)		

		self.btn_Cancelar=ttk.Button(self.ventana_Triaje,text='CANCELAR')
		self.btn_Cancelar['command']=self.ventana_Triaje.destroy
		self.btn_Cancelar.grid(row=10,column=3)
		#self.ventana_Triaje.focus()
		self.ventana_Triaje.grab_set()	
		#self.ventana_Triaje.mainloop()
		...
		try:
			...
			if len(dni)!=0:
				...
				if len(rows)!=0:
					...
					self.Entry_Fecha.insert(0,fecha_Atencion)
					self.Entry_Fecha.configure(state='readonly')
					#self.Entry_Telefono.insert(0,rows_Triaje[0][5])
	...
	def registrar_cita(self):
		...
		if self.controlador and len(consultorio)!=0:
			#print(fecha_A,consultorio,dni)
			rows=self.obj_ConsultaTriaje.query_AgendadoXUsuario(fecha_A,consultorio,dni)
			if len(rows)==0:
				try:
					cup=int(self.etiqueta_Cupo.cget('text'))
					if cup<21 and (id_fuente==1 or id_fuente==2):
						self.obj_ConsultaTriaje.Insert_Cita(id_user,dni,id_fuente,nro_cupo,nro_Referencia,medico,consultorio,fecha_A,telefono,establecimiento,continuador,FUA,Historia,self.turno)
					...
					elif (cup>20 and cup<26) and id_fuente==3:
						self.obj_ConsultaTriaje.Insert_Cita(id_user,dni,id_fuente,nro_cupo,nro_Referencia,medico,consultorio,fecha_A,telefono,establecimiento,continuador,FUA,Historia,self.turno)
						self.borrado_widget1()
						messagebox.showinfo('Notificación','se guardó de manera exitosa')
						self.etiqueta_Cupo.configure(fg='red')
					elif (cup>25 and cup<31) and id_fuente==4:
						self.obj_ConsultaTriaje.Insert_Cita(id_user,dni,id_fuente,nro_cupo,nro_Referencia,medico,consultorio,fecha_A,telefono,establecimiento,continuador,FUA,Historia,self.turno)
						self.borrado_widget1()
						messagebox.showinfo('Notificación','se guardó de manera exitosa')
						self.etiqueta_Cupo.configure(fg='red')
					elif (cup>30 and cup<37) and id_fuente==5:
						self.obj_ConsultaTriaje.Insert_Cita(id_user,dni,id_fuente,nro_cupo,nro_Referencia,medico,consultorio,fecha_A,telefono,establecimiento,continuador,FUA,Historia,self.turno)
						self.borrado_widget1()
						messagebox.showinfo('Notificación','se guardó de manera exitosa')
						self.etiqueta_Cupo.configure(fg='red')
	...
	def search_Citas(self):
		...
		self.ventana_Triaje_Buscar.geometry('1000x400+10+100')	
		...
		self.Entry_dniPaciente.bind('<KeyRelease>',lambda event:self.buscar_Paciente(event))
		#self.Entry_Dni.bind('<Button-1>',self.borrado_widget)
		self.Entry_dniPaciente.grid(row=1,column=1,pady=10)		
		self.ventana_Triaje_Buscar.grab_set()	

		self.table_Historial=ttk.Treeview(self.ventana_Triaje_Buscar,columns=('#1','#2','#3','#4','#5','#6'),show='headings')		
		self.table_Historial.heading("#1",text="DNI")
		self.table_Historial.column("#1",width=100,anchor="center")
		self.table_Historial.heading("#2",text="PACIENTE")
		self.table_Historial.column("#2",width=300,anchor="center")
		self.table_Historial.heading("#3",text="FUENTE")
		self.table_Historial.column("#3",width=100,anchor="center")
		self.table_Historial.heading("#4",text="CONSULTORIO")
		self.table_Historial.column("#4",width=100,anchor="center")		
		self.table_Historial.heading("#5",text="NRO CUPO")
		self.table_Historial.column("#5",width=100,anchor="center")
		self.table_Historial.heading("#6",text="FECHA")
		self.table_Historial.column("#6",width=100,anchor="center")									
		self.table_Historial.place(x=10,y=70,width=1030,height=290)
		
	def buscar_Paciente(self,event):

		dni=''
		dni=dni+self.Entry_dniPaciente.get()				
		...
		for valores in rows:
			...
			#rowsT=self.obj_ConsultaTriaje.consulta_DatosPaciente(dni_paciente)
			if False:
				pass
				#for valor in rowsT:
					#nombres=valor.Nombre+' '+valor.Apellido_Paterno+' '+valor.Apellido_Materno
			else:
				rowsG=self.obj_ConsultaGalen.query_DatosPaciente(dni_paciente)
				if len(rowsG)!=0:
					for valor in rowsG:
						nombres=valor.PrimerNombre+' '+valor.ApellidoPaterno+' '+valor.ApellidoMaterno

			self.table_Historial.insert('','end',values=(dni_paciente,nombres,valores.fuente,valores.Especialidad,valores.Nro_Cupo,valores.Fecha_Atencion))
		
		


#obj_triaje=Triaje()
#obj_triaje.Top_Agregar()